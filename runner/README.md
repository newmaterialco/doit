...
For normal VPS installs, use the installer command generated by the iOS app. It
sets `DOIT_SUPABASE_URL`, `DOIT_SUPABASE_ANON_KEY`, `DOIT_CONNECTOR_TOKEN`, and
`DOIT_HERMES_URL`, then runs:

```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/newmaterialco/doit/main/scripts/install-byo-connector.sh)"
```

The installer clones or updates the repo, creates `runner/.venv`, installs
requirements, writes `/etc/doit/connector.env`, and starts
`doit-connector.service`.

For local development or manual installs, run the connector directly from
`doit/runner` so the nested `runner` Python package resolves correctly:
